(()=>{
  ...
  function validData(value){
    return !!value&&Array.isArray(value.food)&&Array.isArray(value.beverages);
  }
  function currentItems(){return type==="food"?data.food:data.beverages;}
  function categories(){
    return [...new Set(currentItems().filter(x=>x.available!==false).map(x=>x.category))];
  }
  function imageSource(value,fallback){
    ...
    try{
      ...
      if(["https:","http:"].includes(u.protocol))return s;
      if(/^data:image\/(?:jpeg|png|webp|gif);base64,/i.test(s))return s;
    }catch{}
    return fallback;
  }
  function itemKey(x,i,prefix){
    return prefix+":"+(x.id||x.code||((x.category||"")+":"+(typeof x.name==="string"?x.name:x.name?.pt||i)));
  }

  // Patch existing elements instead of replacing the whole menu on refresh.
  // Price-only changes retain the same cards, image nodes and scroll position.
  function compatible(a,b){
    return a&&a.nodeType===b.nodeType&&(a.nodeType!==1||(a.tagName===b.tagName&&(a.getAttribute("data-key")||"")===(b.getAttribute("data-key")||"")));
  }
  ...
  function patchChildren(parent,next){
    ...
    for(const fresh of [...next.childNodes]){
      let node=at;
      const key=fresh.nodeType===1?fresh.getAttribute("data-key"):null;
      if(key&&(!compatible(node,fresh))){
        node=[...parent.children].find(el=>el.getAttribute("data-key")===key&&el.tagName===fresh.tagName)||null;
      }
      if(!compatible(node,fresh)){
        node=fresh.cloneNode(true);parent.insertBefore(node,at);
      }else{
        if(node!==at)parent.insertBefore(node,at);
        patchNode(node,fresh);
      }
      at=node.nextSibling;
    }
    while(at){const next=at.nextSibling;parent.removeChild(at);at=next;}
  }
  function patchHTML(target,html){
    const t=document.createElement("template");t.innerHTML=html;patchChildren(target,t.content);
  }
  function applyHeader(){
    document.documentElement.lang=lang;
    $("pageTitle").textContent=L.title;$("pageSubtitle").textContent=L.sub;
    $("foodBtn").textContent=L.food;$("drinkBtn").textContent=L.drinks;
    document.querySelectorAll(".menu-language-flags a").forEach(a=>a.classList.toggle("active",a.dataset.lang===lang));
  }
  function renderChefSuggestion(){
    const block=data.chefSuggestion,section=$("chefSuggestion");
    const items=(block?.items||[]).filter(x=>x.available!==false);
    section.hidden=type!=="food"||!block||block.enabled===false||!items.length;
    if(section.hidden)return;
    const parts=chefTitleParts[lang]||chefTitleParts.pt;
    $("chefTitleMain").textContent=parts.main;$("chefTitleScript").textContent=parts.script;
    ...
    patchHTML($("chefGrid"),items.map((x,i)=>{
      const name=x.name?.[lang]||x.name?.pt||"",desc=x.description?.[lang]||x.description?.pt||"";
      const image=imageSource(x.image,"assets/dish-placeholder.svg");
      return `<article class="chef-card" data-key="${text(itemKey(x,i,"chef"))}"><div class="chef-card-header"><span class="chef-type">${text(names[x.type]?.[lang]||x.type)}</span><h3>${text(name)}</h3></div><div class="chef-image-wrap"><img class="chef-image zoomable" src="${text(image)}" data-full="${text(image)}" data-caption="${text(name)}" alt="${text(name)}" data-fallback="assets/dish-placeholder.svg"><div class="chef-price">${euro(x.price)}</div></div><div class="chef-card-body"><p>${text(desc)}</p></div></article>`;
    }).join(""));
  }
  function renderCats(){
    const choices=[["all",L.all],...categories().map(k=>[k,cats[k]||k])];
    ...
    patchHTML(list,buttons("floating-cat-btn"));list.scrollLeft=x;
    patchHTML($("categoryStrip"),buttons("cat-btn"));
    updateArrows();
  }
  function renderMenu(){
    const grouped=new Map();
    currentItems().forEach((x,i)=>{
      if(x.available===false||(cat!=="all"&&cat!==x.category))return;
      if(!grouped.has(x.category))grouped.set(x.category,[]);
      grouped.get(x.category).push({x,i});
    });
    const markup=[...grouped].map(([key,items],index)=>{
      const rows=items.map(({x,i})=>{
        const isFood=type==="food";
        const name=isFood?(x.name?.[lang]||x.name?.pt||""):(x.name||"");
        const desc=isFood?(x.description?.[lang]||x.description?.pt||""):(x.region||"");
        const fallback=isFood?"assets/dish-placeholder.svg":"assets/drink-placeholder.svg";
        const image=imageSource(x.image,fallback);
        return `<article class="dish" data-key="${text(itemKey(x,i,type))}"><img class="dish-image${isFood?"":" drink-image"} zoomable" src="${text(image)}" data-full="${text(image)}" data-caption="${text(name)}" alt="${text(name)}" data-fallback="${fallback}"><div><h3>${text(name)}</h3>${desc?`<p>${text(desc)}</p>`:""}</div><div class="price"><span class="price-value">${euro(isFood?x.price:x.bottle)}</span>${!isFood&&x.glass?`<div class="tiny">Copo ${euro(x.glass)}</div>`:""}</div></article>`;
      }).join("");
  ...
  function render(){
    document.body.dataset.menuView=type;document.body.dataset.menuCategory=cat;
    $("foodBtn").classList.toggle("active",type==="food");$("drinkBtn").classList.toggle("active",type==="beverages");
    renderChefSuggestion();renderCats();renderMenu();scheduleBar();
  }
  function updateArrows(){
    const max=Math.max(0,list.scrollWidth-list.clientWidth);
    $("floatingCategoryPrev").disabled=list.scrollLeft<=1;
    $("floatingCategoryNext").disabled=list.scrollLeft>=max-1;
  }
  function updateBar(){
    frame=0;
    const top=parseFloat(getComputedStyle(bar).top)||12;
    const chef=$("chefSuggestion");
    const pastChef=chef.hidden||chef.getBoundingClientRect().bottom<=top+1;
    const inMenu=$("menuArea").getBoundingClientRect().top<=top+bar.offsetHeight+8;
    const visible=list.children.length>1&&pastChef&&inMenu&&!$("imageLightbox").classList.contains("open");
    bar.classList.toggle("is-visible",visible);
    bar.setAttribute("aria-hidden",String(!visible));bar.inert=!visible;
    document.body.classList.toggle("category-bar-visible",visible);
    updateArrows();
  }
  function scheduleBar(){if(!frame)frame=requestAnimationFrame(updateBar);}
  function scrollToMenu(){
    const top=parseFloat(getComputedStyle(bar).top)||12;
    let y=$("menuArea").getBoundingClientRect().top+window.scrollY-top-bar.offsetHeight-4;
    const chef=$("chefSuggestion");
    if(!chef.hidden)y=Math.max(y,chef.getBoundingClientRect().bottom+window.scrollY-top+2);
    window.scrollTo({top:Math.max(0,y),behavior:smooth()});
  }
  function chooseCategory(value){
    if(value!=="all"&&!categories().includes(value))return;
    cat=value;document.body.dataset.menuCategory=cat;renderCats();renderMenu();updateBar();scrollToMenu();
  }
  ...
  function switchType(next){type=next;cat="all";list.scrollLeft=0;render();scrollToMenu();}
  ...
  function closeLightbox(){
    ...
    document.body.classList.remove("modal-open");scheduleBar();
  }
  document.addEventListener("click",e=>{
    const img=e.target.closest("img.zoomable");if(!img)return;
    $("lightboxImage").src=img.dataset.full||img.src;$("lightboxImage").alt=img.alt;
    $("lightboxCaption").textContent=img.dataset.caption||img.alt||"";
    $("imageLightbox").classList.add("open");$("imageLightbox").setAttribute("aria-hidden","false");
    document.body.classList.add("modal-open");updateBar();
  });
  document.addEventListener("error",e=>{
    const img=e.target;
    if(img instanceof HTMLImageElement&&img.dataset.fallback&&img.getAttribute("src")!==img.dataset.fallback){
      img.src=img.dataset.fallback;img.dataset.full=img.dataset.fallback;
    }
  },true);
  ...
  // Capture the current viewport only AFTER the server response, never before:
  // a slow request must not undo a category click or scroll performed meanwhile.
  function applyFreshData(next){
    const signature=JSON.stringify(next);if(signature===lastData)return;
    const scrollY=window.scrollY;
    const anchor=[...document.querySelectorAll("#menuContent .dish,#chefGrid .chef-card")].find(el=>el.getBoundingClientRect().bottom>100);
    const key=anchor?.dataset.key,screenY=anchor?.getBoundingClientRect().top;
    data=next;window.MENU_DATA=next;lastData=signature;render();
    if(!document.body.classList.contains("modal-open")){
      const updated=[...document.querySelectorAll("[data-key]")].find(el=>el.dataset.key===key);
      const desired=updated&&screenY!==undefined?window.scrollY+updated.getBoundingClientRect().top-screenY:scrollY;
      if(Math.abs(window.scrollY-desired)>1)window.scrollTo({top:desired,behavior:"instant"});
    }
    updateBar();
  }
  async function refreshFromServer(force=false){
    if(refreshPromise)return refreshPromise;
    if(!force&&Date.now()-lastRefresh<1000)return;
    lastRefresh=Date.now();
    refreshPromise=(async()=>{
      const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),10000);
      try{
        const res=await fetch(WORKER_DATA_URL+"?_="+Date.now(),{cache:"no-store",signal:controller.signal});
        if(!res.ok)throw new Error("HTTP "+res.status);
        const body=await res.json();if(!body?.ok||!validData(body.data))throw new Error("Invalid menu data");
        applyFreshData(body.data);
      }catch(e){console.warn("Menu: latest data unavailable; keeping displayed content.",e.message);}
  ...
  document.addEventListener("load",scheduleBar,true);
  if(window.ResizeObserver){
    const ro=new ResizeObserver(scheduleBar);ro.observe($("menuArea"));ro.observe($("chefSuggestion"));
  }
  applyHeader();render();updateBar();
